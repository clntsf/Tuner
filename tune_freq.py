...
def tune_cols(filepath, interval, scale, tune_freqs=True, from_params=False, open_output = False):
    ...
    if tune_freqs: scale_notes = construct_note_freqs(scale)
    ...
    for i in range(formants):
        ...
        for j in range(0,vlen,interval):
            ...
            range_freq = (sum(amp_ceils)>0)*sum(nz_freq)/max(1,sum(amp_ceils))
    ...
    clean_out_dir_filepath = sanitize_filepath(out_dir_filepath)
    run(['mkdir', clean_out_dir_filepath], capture_output=True)
    
    # --- Writes the file out to a new .swx file --- #
    out_df.to_csv(f'{out_dir_filepath}/{filename}_tuned.tsv', index=False, sep='\t')
    run(['mv', f'{clean_out_dir_filepath}/{filename}_tuned.tsv', f'{clean_out_dir_filepath}/{filename}_tuned.swx'], capture_output=True)

    # --- Writes the tuning parameters information file --- #
    args=['Tuning Parameters:', '*'*20, f'Interval: {interval}', f'Scale: {(tune_freqs and num_scale_to_strs(scale)) or None}', f'Musicalize: {tune_freqs}']
    with open(f'{out_dir_filepath}/params.txt','w') as writer: writer.write('\n'.join(args))

    if open_output: run(['open', clean_out_dir_filepath], capture_output=True)
    return
